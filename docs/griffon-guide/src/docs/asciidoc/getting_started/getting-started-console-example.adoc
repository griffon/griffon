...
= Console Example
:console-swing-groovy: {rootprojectdir}/samples/console-swing-groovy

== Creating a Project (Gradle)
...
The first step is to get Maven installed on your system. The easiest way to achieve
this goal is to install http://sdkman.io/[SDKMAN] first:
...
Or if your prefer you may install Maven using other means as explained in the official Maven
documentation (link:http://maven.apache.org/download.cgi[Download] and
link:http://maven.apache.org/install.html[Install]).

We're now ready to create the project. You can select any of the Maven archetypes available from
link:https://bintray.com/griffon/griffon-archetypes/[] which currently resolve to the following list
...
Notice that archetypes names follow a naming convention identifying the main UI toolkit
and main programming language. All right, let's create a simple project using Swing as
main UI toolkit, and Groovy as main language:
...
These conventions can be omitted provided you reconfigure the default build files provided by
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/griffon-app/models/console/ConsoleModel.groovy[lines=18..-1]
----
...
Notice the usage of the `@org.kordamp.jipsy.annotations.ServiceProviderFor` annotation. This annotation serves as an additional hint to the
...
is automatically placed under `META-INF/services`. It's contents are fully qualified class names of types that implement
the argument set on the annotation. This results in the following file being automatically created or updated with every
compilation session

.META-INF/services/griffon.core.artifact.GriffonModel
...
files, such as `META-INF/services/griffon.core.artifact.GriffonController`, `META-INF/services/griffon.core.artifact.GriffonView`,
and `META-INF/services/griffon.core.artifact.GriffonService`.
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/griffon-app/controllers/console/ConsoleController.groovy[lines=18..-1]
----
...
`@griffon.annotations.inject.MVCMember` and `@griffon.annotations.core.Nonnull` as hints to the Griffon runtime, enabling
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/src/main/groovy/console/Evaluator.groovy[lines=18..-1]
----

.src/main/groovy/console/GroovyShellEvaluator.groovy
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/src/main/groovy/console/GroovyShellEvaluator.groovy[lines=18..-1]
----

.src/main/groovy/console/ApplicationModule.groovy
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/src/main/groovy/console/ApplicationModule.groovy[lines=18..-1]
----
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/src/main/groovy/console/CenteringWindowDisplayHandler.groovy[lines=18..-1]
----
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/griffon-app/views/console/ConsoleView.groovy[lines=18..-1]
----
<1> MVC member injected by `{link_mvc_group_manager}`
...
[source,groovy,linenums,options="nowrap"]
----
include::{console-swing-groovy}/src/test/groovy/console/ConsoleControllerTest.groovy[lines=18..-1]
----
