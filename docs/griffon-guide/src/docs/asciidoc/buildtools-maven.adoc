...
| org.jacoco               | jacoco-maven-plugin               | 0.7.4.201502262128
| com.zenjava              | javafx-maven-plugin               | 2.0
...
It's very important you take special note of the dependencies ending with `-compile`. As an
application developer these dependencies belong to the _provided_ scope, as these
...
profile before running the `distribution` profile. You *CAN NOT* combine
both profiles.
...
This profile creates an universal installer using {link_izpack}.
Enable it with `maven -Pizpack`. You must execute the `distribution`
profile before running the `izpack` profile. You *CAN NOT* combine
both profiles.
...
This profile crates a single JAR by combining the application's classes
