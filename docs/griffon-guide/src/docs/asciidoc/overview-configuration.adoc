...
this feat by registering an appropriate implementation of `{configuration_decorator_factory}`, for example
`{mutable_configuration_decorator_factory}`. You'll have to register this class with a module
