...
mechanism provided by the `com.googlecode.openbeans` package. The default `{link_resources_injector}` queries
...
PropertyEditors from the following classpath resource: `/META-INF/services/com.googlecode.openbeans.PropertyEditor`.
