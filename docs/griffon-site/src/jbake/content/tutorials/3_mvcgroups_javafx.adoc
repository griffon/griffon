...
[source]
----
$ lazybones create griffon-javafx-java editor
Creating project from template griffon-javafx-java (latest) in 'editor'
...
----
include::{editor-javafx-java}/src/main/java/editor/Document.java[lines=16..-1]
----
...
----
include::{editor-javafx-java}/src/main/java/editor/DocumentModel.java[lines=16..-1]
----
...
----
include::{editor-javafx-java}/griffon-app/models/editor/ContainerModel.java[lines=16..-1]
----
...
icon:bolt[] The final remark on this model class is that it implements the `javafx.beans.value.ChangeListener` interface.
In this way the model instance can listen to tab selection changes and update the _documentModel_ property.

icon:bolt[] Let's mode to the View. Open up `ContainerView.java` and paste the following into it

[source,java,options="nowrap"]
.griffon-app/views/editor/ContainerView.java
----
include::{editor-javafx-java}/griffon-app/views/editor/ContainerView.java[lines=16..-1]
----
...
This file defines a `MenuBar` and a the tab container (a `JTabbedPane`) name _tabGroup_. This tab container is exposed
to the outside world via a getter method; we'll see why it's done this way when the second MVC group comes into play.
The View is also responsible for managing a `FileChooser` that will be used to select files for reading. Also, the
`model` is registered as a change listener with the _tabGroup_ field.
...
----
include::{editor-javafx-java}/griffon-app/i18n/messages.properties[lines=17..-1]
----
...
----
include::{editor-javafx-java}/griffon-app/conf/Config.java[lines=16..-1]
----
...
----
include::{editor-javafx-java}/griffon-app/models/editor/EditorModel.java[lines=16..-1]
----
...
icon:bolt[] The `EditorView` relies on FXML too create the nodes.

[source,java,options="nowrap"]
.griffon-app/views/editor/EditorView.java
----
include::{editor-javafx-java}/griffon-app/views/editor/EditorView.java[lines=16..-1]
----
...
Take special note in the the usages of the _parentView_ field. Notice the that the type is set to `ContainerView`,
this means this View is aware that it's not a top level MVC group, that is, there will be another MVC group that
will be responsible for instantiating `editor`, in this case is the `container` MVC group. A parent/child
...
----
include::{editor-javafx-java}/griffon-app/controllers/editor/EditorController.java[lines=16..-1]
----
...
.griffon-app/controllers/editor/ContainerController.java
----
include::{editor-javafx-java}/griffon-app/controllers/editor/ContainerController.java[lines=16..-1]
----
...
link:https://github.com/griffon/griffon/tree/master/samples/editor-javafx-java[here, window="_blank"].
