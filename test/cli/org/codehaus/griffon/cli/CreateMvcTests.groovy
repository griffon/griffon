...
class CreateMvcTests extends AbstractCliTests {
    ...
    void testCreatesDirectories() {
        ...
        System.setProperty("griffon.cli.args", "Book -skipPackagePrompt=true")
        gantRun("CreateMvc")
    ...
    void tryMvc(String name) {
        ...
        System.setProperty("griffon.cli.args", "$name -skipPackagePrompt=true")
        gantRun("CreateMvc")
        ...
        //assert applicationConf.text =~ "mvcGroups\\s*\\{\\s*(//[^\n]*\n\\s*)?'$name'\\s*\\{\\s*model\\s*=\\s*'${fqn}Model'\\s*controller\\s*=\\s*'${fqn}Controller'\\s*view\\s*=\\s*'${fqn}View'\\s*\\}"
    }
}
