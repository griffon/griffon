...
pluginDiscoveryRepositories = [default:DEFAULT_PLUGIN_DIST]
if(buildConfig?.griffon?.plugin?.repos?.discovery) {
    pluginDiscoveryRepositories.putAll(buildConfig?.griffon?.plugin?.repos?.discovery)
}

// setup default plugin respositories for publishing
pluginDistributionRepositories = [default:DEFAULT_PUBLISH_URL]
if(buildConfig?.griffon?.plugin?.repos?.distribution) {
    pluginDistributionRepositories.putAll(buildConfig?.griffon?.plugin?.repos?.distribution)
}
 ...
 * The "default" repo is ignored (see above for default configuration)
...
public Map configureAuth(Map repoMap,String repoType) {
    ...
    repoMap.each{
        ...
        // else no authentication manager provides to authManagerMap : case of DEFAULT_PUBLISH_URL
        } else {
            if(isSecureUrl(it.value)) {
...
// Now check whether the plugin is in the repository.
    ...
    if(authManager != null) repos.authenticationManager = authManager
    boolean notInRepository = true
    try {
        notInRepository = !repos.info("griffon-$pluginName", -1)
    }
    catch (e) {
        // ignore
    }
...
target(importToSVN:"Imports a plugin project to Griffon' remote SVN repository") {
    ...
    ant.mkdir(dir:"${basedir}/unzipped/griffon-app")
    ...
    ant.copy(todir:testsDir) {
        fileset(dir:"${griffonSettings.testSourceDir}")
    }
...
def buildPluginInfo(root, pluginName) {
    use(DOMCategory) {
        ...
        // determine if this is a secure plugin spot..
        def tagsUrl = "${pluginDistURL}/griffon-${pluginName}/tags"
        try {
            if (shouldUseSVNProtocol(pluginDistURL)) {
        ...
        try {
            ...
            def url = "${pluginDistURL}/griffon-${pluginName}/tags/LATEST_RELEASE/plugin.xml"
