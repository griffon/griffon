 ...
 * @since 3.0.0
 */
public class Dimension2DConverter extends AbstractConverter<Dimension2D> {
    @Override
    public Dimension2D fromObject(Object value) throws ConversionException {
        if (null == value) {
            return null;
    ...
    @Override
    public String toString(Dimension2D value) throws ConversionException {
        if (null == value) { return null; }
        return value.getWidth() + ", " + value.getHeight();
    }


    protected Dimension2D handleAsString(String str) {
        if (isBlank(str)) {
            return null;
        }
    ...
    protected Dimension2D handleAsList(List<?> list) {
        if (list.isEmpty()) {
            return null;
        }
    ...
    protected Dimension2D handleAsMap(Map<?, ?> map) {
        if (map.isEmpty()) {
            return null;
        }
    ...
    protected Dimension2D handleAsNumber(Number value) {
