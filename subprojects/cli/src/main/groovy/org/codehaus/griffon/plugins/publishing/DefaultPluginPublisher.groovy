 ...
 * when publishing plugins to a Griffon plugin repository
 *
...
public class DefaultPluginPublisher {
    ...
    GPathResult publishRelease(String pluginName, Resource pluginsList, boolean makeLatest) {
        ...
        def props = ['title', 'author', 'authorEmail', 'description', 'documentation', 'license', 'toolkits', 'platforms']
        def releaseInfo = {
            release([tag:releaseTag,version:pluginVersion, type:'svn']) {
                for(p in props) {
                    if(releaseMetadata."$p") "$p"(releaseMetadata."$p".text())
                }
                //if(releaseMetadata.description) synopsis(releaseMetadata.description.text().trim())
                file "$repositoryURL/griffon-$pluginName/tags/$releaseTag/griffon-$pluginName-${pluginVersion}.zip"
            }
        }
