...
''')
    ...
    void testConfigReaderNestedValues() {
        def config = new ConfigReader(new DefaultApplicationClassLoader()).parse('''
foo {
    bar {
        password="value"
...
''')
        ...
        config = new ConfigReader(new DefaultApplicationClassLoader()).parse('''
            foo {
                bar.password="value"
                fruit="orange"
            }
    ...
    void testConfigBinding() {
        def reader = new ConfigReader(new DefaultApplicationClassLoader())
        reader.binding = [foo: "bar"]
...
test=foo + 1        
    ...
    void testEnvironmentProperties2() {
        ConfigReader reader = new ConfigReader(new DefaultApplicationClassLoader())
        reader.setEnvironment('production')
        def config = reader.parse('''
dataSource {
    pooling = false
...
environments {
    ...
    void testParseProperties() {
        ...
        def config = new ConfigReader(new DefaultApplicationClassLoader()).parse(props)
...
''')
    ...
    void testScopedVariableReusage() {
        ...
        def config = new ConfigReader(new DefaultApplicationClassLoader()).parse(conf)
...
environments {
    development {
        ...
        reader.setEnvironment("development")
        config = reader.parse('''
log4j {
    appender {
        stdout("org.apache.log4j.ConsoleAppender") {        
...
log4j {
    ...
    void testNotProperlyNestedPropertiesArePreserved() throws IOException {
        Properties props = new Properties()
        props.load(ConfigReaderTest.class.getResourceAsStream("/griffon/core/util/groovy/system.properties"))
        assertEquals("false", props.get("catalog.prov"))
        assertEquals("sa", props.get("catalog.prov.db.user"))

        // now round-trip via ConfigReader
        ConfigReader configReader = new ConfigReader(new DefaultApplicationClassLoader())
        ConfigObject newConfig = configReader.parse(props)
    ...
    void testSameElementNestingWithoutDuplication() {
        def cfg = """ 
            ...
            a { b { a { bar = 3 } } }
        """
        ConfigObject c = new ConfigReader(new DefaultApplicationClassLoader()).parse(cfg)
     ...
     * ConfigReader only allows a single block for any given name
     */
