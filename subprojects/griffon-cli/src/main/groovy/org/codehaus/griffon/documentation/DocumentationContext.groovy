...
public class DocumentationContext {
    ...
    DocumentationContext documentMethod(String artefact, Class type, String name, Class[] arguments) {
        if(!currentDocumentation) {
            if(GriffonClassUtils.isGetter(name, arguments)) {
                currentDocumentation = properties.find { it.name == GriffonClassUtils.getPropertyForGetter(name) }?.text
            }
            if(GriffonClassUtils.isSetter(name, arguments)) {
                currentDocumentation = properties.find { it.name == GriffonClassUtils.getPropertyForSetter(name) }?.text
            }
        }
    ...
    DocumentationContext documentProperty(String artefact, Class type, String name) {
        if(!currentDocumentation) {
            def getterOrSetter = methods.find {it.name == GriffonClassUtils.getGetterName(name) || it.name == GriffonClassUtils.getSetterName(name)}
