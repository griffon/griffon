 ...
 * <p/>
 * This is carbon copy of ResolveVisitor as of Groovy 1.7.4; needed in order to inject
 * additional imports per each Griffon artifact, due to ResolveVisitor being too closed.
 *
...
public class ResolveVisitor extends ClassCodeExpressionTransformer {
    ...
    private Map<ClassNode, String[]> additionalImports = new LinkedHashMap<ClassNode, String[]>();
    ...
    public Map<ClassNode, String[]> getAdditionalImports() {
    ...
    private boolean resolveFromDefaultImports(ClassNode type, boolean testDefaultImports) {
        ...
        if (testDefaultImports) {
            String[] otherImports = additionalImports.get(currentClass);
            if (otherImports != null) {
                for (int i = 0, size = otherImports.length; i < size; i++) {
                    String name = type.getName();
                    // Let the standard matching mechanism deal with this one
                    // as java.awt.List <=> java.util.List
                    if("List".equals(name)) break;
                    String packagePrefix = otherImports[i];
