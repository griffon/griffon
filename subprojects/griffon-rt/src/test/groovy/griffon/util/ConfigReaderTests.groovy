...
test=foo + 1        
    ...
    void testEnvironmentProperties2() {
        def config = new ConfigReader("production").parse('''
dataSource {
    pooling = false
...
environments {
    ...
    void testParseProperties() {
        ...
        def config = new ConfigReader().parse(props)
...
''')
    ...
    void testScopedVariableReusage() {
        ...
        def config = new ConfigReader().parse(conf)
...
log4j {
    ...
    void testSameElementNestingWithoutDuplication() {
        def cfg = """ 
            ...
            a { b { a { bar = 3 } } }
        """
        ConfigObject c = new ConfigReader().parse(cfg)
