...
[[_getting_started_sample_applications]]
= Sample Applications
...
Instances of link:api/griffon/core/artifact/GriffonModel.html[GriffonModel] implement
the link:api/griffon/core/Observable.html[Observable], interface which means they know
how to handle observable properties out of the box. We only need to be concerned in
...
conventions and applying the appropriate byte code manipulation (via AST transformations).
 The compiled Model class does implement the +link:api/griffon/core/artifact/GriffonModel.html[GriffonModel]+
interface as required by the framework. This type of byte code manipulation is
expected to work for every Groovy based artifact.
...
Similarly to +@Bindable+ we find that Groovy based JavaFX models can use another AST
