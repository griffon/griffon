...
MVC groups configuration is setup in +Config.groovy+ located inside +griffon-app/conf+.
This file holds an entry for every MVC group that the application has (not counting
those provided by <<_addons,Addons>>).
...
relies on calling +link:api/griffon/core/artifact/GriffonArtifact.html#mvcGroupInit[mvcGroupInit()]+
on the group member.
...
+link:api/griffon/core/artifact/GriffonMVCArtifact.html#mvcGroupInit[mvcGroupInit()]+,
as part of the arguments sent to that method. Here's how the +FooController+ can
reach the key defined in the configuration
