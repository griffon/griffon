...
Actions must follow these rules in order to be considered as such:
 ...
 - must pass +link:api/griffon/util/GriffonClassUtils.html#isPlainMethod[GriffonClassUtils.isPlainMethod()]+.
 - must have +void+ as return type.

The application's +link:api/griffon/core/controller/ActionManager.html[ActionManager]+
will automatically configure an +link:api/griffon/core/controller/Action.html[Action]+
...
Controllers can perform other tasks too:
 ...
 - listen to link:overview-application-events/[application events].
 - react to MVC initialization/destruction via a pair of methods (+mvcGroupInit()+, +mvcGroupDestroy()+).
 - hold references to link:services/[services].
...
include::controllers-action-manager.adoc[]
