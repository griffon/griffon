...
either properties files or Groovy scripts, please refer to the link:resourceResolverConfiguration[configuration] section.
...
a +String+ from the first two formats. You'll have to rely on link:propertyEditors[property editors]
in order to transform the value into the correct type. link:injectedResources[Injected resources]
