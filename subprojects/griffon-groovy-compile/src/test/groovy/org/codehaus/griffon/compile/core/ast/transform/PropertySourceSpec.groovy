...
class PropertySourceSpec extends Specification {
    def 'PropertySourceASTTransformation is applied to a bean via @PropertySource'() {
        given:
        GroovyShell shell = new GroovyShell()

        when:
        def bean = shell.evaluate('''
            @griffon.annotations.beans.PropertySource
            class Bean { }
            new Bean()
            ''')

        then:
        bean instanceof PropertySource
        PropertySource.methods.each { Method target ->
