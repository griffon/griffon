    ...
    public void visit(ASTNode[] nodes, SourceUnit sourceUnit) {
        ...
        if (parent instanceof FieldNode) {
            ...
            if ((modifiers & Modifier.FINAL) != 0) {
                String msg = "@griffon.transform.javafx.FXObservable cannot annotate a final property.";
    ...
    private void addJavaFXProperty(SourceUnit source, AnnotationNode node, ClassNode declaringClass, FieldNode field) {
        ...
        for (PropertyNode propertyNode : declaringClass.getProperties()) {
            if (propertyNode.getName().equals(fieldName)) {
                if (field.isStatic()) {
                    String message = "@griffon.transform.javafx.FXObservable cannot annotate a static property.";
        ...
        String message = "@griffon.transform.javafx.FXObservable must be on a property, not a field. Try removing the private, " +
