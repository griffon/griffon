...
/**
 * Handles generation of code for the {@code @InvalidationListener} annotation.
 * <p>
...
public class InvalidationListenerASTTransformation extends AbstractASTTransformation implements AnnotationHandler {
    ...
    /**
     * Convenience method to see if an annotated node is {@code @InvalidationListener}.
     *
    ...
    public static boolean hasListenerAnnotation(AnnotatedNode node) {
        for (AnnotationNode annotation : node.getAnnotations()) {
            if (INVALIDATION_LISTENER_CLASS.equals(annotation.getClassNode())) {
                return true;
            }
        }
        return false;
    ...
    private static void processExpression(ClassNode classNode, String propertyName, Expression expression, boolean useWeakListener) {
        if (expression instanceof ClosureExpression) {
            addInvalidationListener(classNode, propertyName, (ClosureExpression) expression, useWeakListener);
        } else if (expression instanceof VariableExpression) {
            addInvalidationListener(classNode, propertyName, (VariableExpression) expression, useWeakListener);
        } else if (expression instanceof ConstantExpression) {
            addInvalidationListener(classNode, propertyName, (ConstantExpression) expression, useWeakListener);
        } else {
            throw new RuntimeException("Internal error: wrong expression type. " + expression);
        }
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, ClosureExpression closure, boolean useWeakListener) {
        ArgumentListExpression args = createListenerExpression(classNode, propertyName, CastExpression.asExpression(JAVAFX_INVALIDATION_LISTENER_CLASS, closure), useWeakListener);
        addListenerStatement(classNode, propertyName, args);
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, VariableExpression variable, boolean useWeakListener) {
        ArgumentListExpression args = createListenerExpression(classNode, propertyName, CastExpression.asExpression(JAVAFX_INVALIDATION_LISTENER_CLASS, variable), useWeakListener);
        addListenerStatement(classNode, propertyName, args);
    }
    ...
    private static ArgumentListExpression createListenerExpression(ClassNode classNode, String propertyName, Expression expression, boolean useWeakListener) {
        ...
        if (useWeakListener) {
            String fieldName = "$" + propertyName + "InvalidationListener__" + System.nanoTime();
            FieldNode listenerField = new FieldNode(
                fieldName,
                PRIVATE | FINAL,
                JAVAFX_INVALIDATION_LISTENER_CLASS,
                classNode,
                expression);
            ...
            args.addExpression(
                ctor(JAVAFX_WEAK_INVALIDATION_LISTENER_CLASS, params)
            );
        } else {
            args.addExpression(CastExpression.asExpression(JAVAFX_INVALIDATION_LISTENER_CLASS, expression));
        }
        return args;
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, ConstantExpression reference, boolean useWeakListener) {
        addInvalidationListener(classNode, propertyName, new VariableExpression(reference.getText()), useWeakListener);
    }
