...
public class InvalidationListenerASTTransformation extends AbstractASTTransformation {
    ...
    private static void processExpression(ClassNode classNode, String propertyName, Expression expression) {
        if (expression instanceof ClosureExpression) {
            addInvalidationListener(classNode, propertyName, (ClosureExpression) expression);
        } else if (expression instanceof VariableExpression) {
            addInvalidationListener(classNode, propertyName, (VariableExpression) expression);
        } else if (expression instanceof ConstantExpression) {
            addInvalidationListener(classNode, propertyName, (ConstantExpression) expression);
        } else {
            throw new RuntimeException("Internal error: wrong expression type. " + expression);
        }
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, ClosureExpression closure) {
        ArgumentListExpression args = new ArgumentListExpression();
        args.addExpression(CastExpression.asExpression(JAVAFX_INVALIDATION_LISTENER_CLASS, closure));
        addListenerStatement(classNode, propertyName, args);
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, VariableExpression variable) {
        ArgumentListExpression args = new ArgumentListExpression();
        args.addExpression(CastExpression.asExpression(JAVAFX_INVALIDATION_LISTENER_CLASS, variable));
        addListenerStatement(classNode, propertyName, args);
    }

    private static void addInvalidationListener(ClassNode classNode, String propertyName, ConstantExpression reference) {
        ArgumentListExpression args = new ArgumentListExpression();
