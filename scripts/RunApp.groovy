...
target(runApp: "Runs the application from the command line") {
    ...
    Process p = "$javaVM -classpath ${runtimeJars.collect {f -> f.name}.join(File.pathSeparator)} $proxySettings $javaOps griffon.application.StandaloneApplication".execute(null as String[], jardir)
